...
def letterCounts(fname):
    ...
    with open(fname, "r") as f:
        value = f.read()
    count_nonletter = 0
    #This counts the number of each char from the file adds it into a dict
    for char in value:
        if char == letter(char):
            if char not in Count:
                ...
                valuechar = Count[char]
                valuechar += 1
                Count[char] = valuechar
        else:
            count_nonletter += 1
            Count["non-letter"] = count_nonletter

    #This converts the Count dict to tuple the its sorts and reverse the tuple into descending order
    lst = list(Count.items())
    newlst=[]
    for i in lst:
        x = list(i)
        x.reverse()
        newlst.append(tuple(x))
        newlst.sort()
        newlst.reverse()
    return newlst
